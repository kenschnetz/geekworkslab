    ...
    <div class="flex flex-col pt-8">
        @if(count($posts) > 0)
            @foreach($posts as $post)
                <a class="inline-block" href="{{route('post', ['category_slug' => $post->Category->slug, 'post_slug' => $post->slug])}}">
                    <div wire:key="row-{{ $post->id }}">
                        <hr />
                        <div class="flex items-center p-4 cursor-pointer hover:bg-gray-100">
                            ...
                            <div class="w-full">
                                ...
                                <div class="text-center md:text-left text-lg italic">
                                    {{ strip_tags(Str::limit($post->description), '<br>') }}
                                </div>
                                @if(!empty(optional($post->Images->first())->path))
                                    <div class="md:hidden">
                                ...
                                <div class="py-1">
                                    {!! strip_tags(Str::limit($post->content, 200), '<br>') !!}
                                </div>
                            </div>
                        </div>
                    </div>
